...
class TicTacToe:
    def __init__(self):
        ...
        pygame.display.set_caption("Tic Tac Toe Evolution - Simplified")
        ...
        for player in [Player.X, Player.O]:
            ...
            for col in range(self.board_size):
                if all(self.board[row][col] == player for row in range(self.board_size)):
                    if self.count_consecutive([self.board[row][col] for row in range(self.board_size)]) >= self.win_requirements['vertical']:
                        return player
    ...
    def get_winning_line(self):
        ...
        # Simplified - return the first winning line found
        for player in [Player.X, Player.O]:
            # Check rows
            for row in range(self.board_size):
                if all(self.board[row][col] == player for col in range(self.board_size)):
