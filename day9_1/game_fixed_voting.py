...
class TicTacToe:
    def __init__(self):
        ...
        pygame.display.set_caption("Tic Tac Toe Evolution - Fixed Voting")
        ...
        for player in [Player.X, Player.O]:
            ...
            for col in range(self.board_size):
                if all(self.board[row][col] == player for row in range(self.board_size)):
                    if self.count_consecutive_for_player([self.board[row][col] for row in range(self.board_size)], player) >= self.win_requirements['vertical']:
                        return player
    ...
    def count_consecutive_for_player(self, line, player):
        """Count consecutive pieces for a specific player"""
        max_count = 0
        current_count = 0
    ...
    def handle_vote_click(self, pos):
        """Handle clicks during voting - FIXED VERSION"""
        if not self.vote_options:
            return
            
        x, y = pos
        
        # Check if click is in voting area (below title)
        if y < 150:  # Title area
            return
            
        # Calculate which modifier was clicked
        modifier_index = (y - 150) // 120
        
        if 0 <= modifier_index < len(self.vote_options):
            chosen = self.vote_options[modifier_index]
            print(f"Selected modifier: {chosen}")  # Debug output
            self.apply_modifier(chosen)
            self.next_round()
            
    def apply_modifier(self, modifier_type):
        """Apply a modifier to the game"""
        if modifier_type == 'board_expansion' and self.board_size < 4:  # Fixed: only expand to 4x4
            self.board_size += 1
            self.board = [[None]*self.board_size for _ in range(self.board_size)]
            print(f"Board expanded to {self.board_size}x{self.board_size}")  # Debug output
    ...
    def draw_title(self):
        ...
        title = self.fonts['large'].render("Tic Tac Toe Evolution", True, self.theme['text'])
    ...
    def draw_info(self):
        ...
        # Board size (debug info)
        board_size_text = self.fonts['small'].render(f"Board: {self.board_size}x{self.board_size}", True, self.theme['text_secondary'])
        self.screen.blit(board_size_text, (WINDOW_SIZE[0] - 200, 100))
    ...
    def draw_voting(self):
        ...
        if self.vote_options:
            for i, modifier_type in enumerate(self.vote_options):
                ...
                # Click instruction
                click_text = self.fonts['small'].render("Click to select", True, self.theme['highlight'])
                self.screen.blit(click_text, (70, 230 + i * 120))
                
    def draw_end(self):
        """Draw the end game screen"""
        title = self.fonts['large'].render("Game Over!", True, self.theme['text'])
        title_rect = title.get_rect(center=(WINDOW_SIZE[0]//2, WINDOW_SIZE[1]//3))
        self.screen.blit(title, title_rect)
        
        # Final scores
        x_final = self.fonts['medium'].render(f"X: {self.scores[Player.X]}", True, self.theme['x'])
        o_final = self.fonts['medium'].render(f"O: {self.scores[Player.O]}", True, self.theme['o'])
        
        x_rect = x_final.get_rect(center=(WINDOW_SIZE[0]//2 - 100, WINDOW_SIZE[1]//2))
        o_rect = o_final.get_rect(center=(WINDOW_SIZE[0]//2 + 100, WINDOW_SIZE[1]//2))
        
        self.screen.blit(x_final, x_rect)
        self.screen.blit(o_final, o_rect)
        
        # Winner announcement
        if self.scores[Player.X] > self.scores[Player.O]:
            winner_text = self.fonts['large'].render("X Wins the Game!", True, self.theme['success'])
        elif self.scores[Player.O] > self.scores[Player.X]:
            winner_text = self.fonts['large'].render("O Wins the Game!", True, self.theme['success'])
        else:
            winner_text = self.fonts['large'].render("It's a Tie!", True, self.theme['highlight'])
            
        winner_rect = winner_text.get_rect(center=(WINDOW_SIZE[0]//2, WINDOW_SIZE[1]//2 + 100))
        self.screen.blit(winner_text, winner_rect)
        
        # Restart instruction
        restart_text = self.fonts['medium'].render("Click anywhere to play again", True, self.theme['text_secondary'])
        restart_rect = restart_text.get_rect(center=(WINDOW_SIZE[0]//2, WINDOW_SIZE[1] - 100))
