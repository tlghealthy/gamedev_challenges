...
class TicTacToe:
    def __init__(self):
        ...
        pygame.display.set_caption("Tic Tac Toe Evolution - Fixed")
    ...
    def check_winner(self):
        """Check for a winner considering modifier effects - FIXED VERSION"""
    ...
    def count_consecutive(self, line):
        """Count consecutive pieces of the same player - FIXED VERSION"""
        max_count = 0
        current_count = 0
        current_player = None
    ...
    def get_winning_line(self):
        """Get the coordinates of the winning line - FIXED VERSION"""
        for player in [Player.X, Player.O]:
            # Check rows
            for row in range(self.board_size):
                if self.count_consecutive(self.board[row]) >= self.win_requirements['horizontal']:
    ...
    def draw_title(self):
        ...
        title = self.fonts['large'].render("Tic Tac Toe Evolution - Fixed", True, self.theme['text'])
